...
\textbf{Proof Strategy:} We prove by structural induction on face segments divided by existing chords. The key insight is that existing chords partition the face into smaller regions, and by analyzing the smallest regions (1 or 2 internal vertices), we can guarantee safe roots exist. The proof handles two cases: (1) no blocking chords exist (trivial—all vertices are safe), or (2) chords exist and partition the face into regions where boundary vertices of smallest regions are provably safe.
...
\textbf{Proof Strategy:} We prove completeness by showing: (1) every valid triangulation appears in some tree path from a safe root, and (2) the pruning of invalid branches (those creating multi-edges) is sound—all descendants of pruned branches would also be invalid. The no-duplication property follows from the uniqueness of the generating chord (leftmost chord) in each triangulation, ensuring each triangulation appears exactly once in the genealogical tree structure.
...
\begin{proof}
Consider face $F_i$ with $n_i \geq 5$ vertices labeled $v_1, v_2, \ldots, v_{n_i}$ in counterclockwise order. Let $S$ be the global chord set containing all chords from previously triangulated faces.

\textbf{Case Analysis by Safe Root Selection:}

By Theorem~\ref{thm:safe_root_exists}, face $F_i$ has at least two safe root vertices. Let $r$ be one such safe root vertex. Without loss of generality, assume $r = v_1$ (we can relabel vertices accordingly).

Since $v_1$ is a safe root, the algorithm can construct the root triangulation $T_{ir}$ by adding chords from $v_1$ to all non-adjacent vertices: $(v_1, v_3), (v_1, v_4), \ldots, (v_1, v_{n_i-1})$. This gives us at least one triangulation.

\textbf{Generating a Second Triangulation:}

From the root triangulation $T_{ir}$, we can perform a flip operation to generate at least one child triangulation. Specifically:

\begin{enumerate}
    \item The root triangulation contains chord $(v_1, v_{n_i-1})$ which is the rightmost chord from $v_1$.
    
    \item This chord is part of a triangle with vertices $v_1, v_{n_i-1}, v_{n_i}$ (where $v_{n_i}$ is adjacent to both $v_1$ and $v_{n_i-1}$ on the face boundary).
    
    \item We can flip to create chord $(v_2, v_{n_i})$ instead. This is valid because:
    \begin{itemize}
        \item $(v_2, v_{n_i}) \notin S$: If $(v_2, v_{n_i})$ were in $S$, then $v_1$ would not be a safe root (contradiction).
        \item The flip does not create multi-edges: The quadrilateral $v_1v_2v_{n_i}v_{n_i-1}$ allows the flip.
    \end{itemize}
    
    \item For $n_i \geq 5$, vertices $v_2$ and $v_{n_i}$ are non-adjacent on the original face boundary (they are separated by at least 3 edges: $v_2 \to v_3 \to \cdots \to v_{n_i-1} \to v_{n_i}$). Therefore, $(v_2, v_{n_i})$ is a valid chord.
\end{enumerate}

This flip operation produces a distinct triangulation $T_{ir}' = T_{ir} \setminus \{(v_1, v_{n_i-1})\} \cup \{(v_2, v_{n_i})\}$.

\textbf{Verification for $n_i = 5$:}

For the smallest case $n_i = 5$:
\begin{itemize}
    \item Root triangulation from $v_1$: chords $(v_1, v_3), (v_1, v_4)$
    \item Flip chord $(v_1, v_4)$ to create chord $(v_2, v_5)$
    \item New triangulation: chords $(v_1, v_3), (v_2, v_5)$
\end{itemize}

Both triangulations are valid and distinct, proving $d_i \geq 2$ for $n_i = 5$.

For $n_i > 5$, additional flip operations provide even more triangulations, so $d_i \geq 2$ holds.

\textbf{Worst-Case Scenario:}

Even in the worst case where the global chord set $S$ contains many blocking chords:
\begin{itemize}
    \item At least one safe root exists (by Theorem~\ref{thm:safe_root_exists})
    \item From that root, at least two triangulations can be constructed as shown above
    \item Therefore, $d_i \geq 2$ for all $n_i \geq 5$
\end{itemize}
\end{proof}
...
\textbf{Case k.1:} If $n_k = 4$, then $d_k \in \{1, 2\}$:
\begin{itemize}
    \item If $d_k = 1$: Then $F_k = 2 \cdot d_1 \cdots d_{k-1} + d_1 \cdots d_{k-1} \cdot 1 = 3 \cdot d_1 \cdots d_{k-1} < 4T$
    \item If $d_k = 2$: Then $2 \cdot d_1 \cdots d_{k-1} \leq d_1 \cdots d_k$, so $F_k \leq 2T$
\end{itemize}
