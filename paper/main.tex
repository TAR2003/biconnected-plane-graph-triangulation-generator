...
\textbf{Average-case vs. Worst-case Complexity:}\footnote{Hash set operations are $O(1)$ average-case with high probability under the random oracle model. In our implementation, we use Java's \texttt{HashSet} with a prime-sized table and handle collisions via separate chaining. Worst-case $O(n)$ lookup occurs only if all chords hash to the same bucket, which has negligible probability with a proper hash function. Our prime-based hash function ensures uniform distribution in practice. See Section~\ref{sec:hash_analysis} of the supplementary materials for detailed collision analysis.} When we state that hash set operations take $O(1)$ time, this refers to \emph{average-case} complexity under standard assumptions about hash function quality. In the worst case (all elements collide), operations degrade to $O(n)$. However, our implementation uses:
\begin{itemize}
