...
\subsection{Comparison: Triconnected vs. Biconnected Approach (Original Text)}
...
\textbf{Algorithm Flow:} The algorithms work together in a hierarchical fashion: Algorithm 2 calls Algorithm 3, which calls Algorithm 4, which calls Algorithm 5. Algorithm 5 recursively calls itself and Algorithm 6. Algorithm 6 may recursively call Algorithm 3 for the next face. Algorithms 1 and 7 are utility functions called as needed.
...
\begin{itemize}
    \item Let $e'$ be the \textbf{unique} chord in $C$ that intersects with $e$ (forming a quadrilateral with $e$). This uniqueness follows from planarity: in a triangulated face, any chord $e$ can intersect at most one other chord $e'$, because the triangulation partitions the face into non-overlapping triangles. If two distinct chords both intersected $e$, they would violate the planar embedding constraint.
...
This notation appears throughout Algorithms~\ref{alg:face_triangulation} and~\ref{alg:child_triangulations}.
\end{definition}
...
\textbf{Explanation of Line 7 Condition:} The condition ``$(v_1, v_j)$ is a chord of $T_i$ \textbf{or} $(v_1, v_j) \notin S$'' is a \emph{disjunctive condition} that determines when it's safe to explore a child triangulation by flipping to create/use chord $(v_1, v_j)$. This is NOT an exclusive choice—the algorithm explores the branch whenever \emph{at least one} of these conditions holds:

\begin{itemize}
    \item \textbf{First condition: ``$(v_1, v_j)$ is a chord of $T_i$''}---The chord $(v_1, v_j)$ already exists in the current triangulation $T_i$. This means we can perform an edge flip operation: flip away the generating chord and replace it with $(v_1, v_j)$ (or a related configuration). Since the chord already exists locally, no multi-edge conflict can occur.
    
    \item \textbf{Second condition: ``$(v_1, v_j) \notin S$''}---The chord $(v_1, v_j)$ does \emph{not} exist in the global chord set $S$ (i.e., it hasn't been added by any previously triangulated face). This means adding this chord to the current face triangulation will not create a multi-edge. We can safely add it.
    
    \item \textbf{When both conditions are true:} If $(v_1, v_j)$ is already a chord in $T_i$ \emph{and} doesn't conflict with $S$, we explore that branch (it satisfies both conditions, so it's definitely safe).
    
    \item \textbf{When exactly one condition is true:} If only one condition holds, we still explore the branch (the disjunction ensures we don't miss valid children).
    
    \item \textbf{When neither condition is true:} If $(v_1, v_j)$ is \emph{not} in $T_i$ \emph{and} $(v_1, v_j) \in S$ (already in global set), then adding it would create a multi-edge. We skip this branch (pruning invalid descendants).
\end{itemize}

\textbf{Key Insight:} The algorithm explores \emph{all} valid branches (every $j$ for which the condition holds), not a single chosen branch. This ensures completeness—every reachable triangulation in the local genealogical tree is visited.
...
If $v_p = r$, then flipping $(v_a, v_b)$ would create chord $(r, v_q)$. But all chords emanating from the root $r$ are established in the root triangulation and never created through flipping in descendant triangulations (by the definition of the local genealogical tree structure).

Creating a new chord from $r$ in a descendant would violate the fundamental tree construction rule.

Therefore, $(v_a, v_b)$ cannot be the generating chord in this case (violates the tree structure constraint).
...
\end{tikzpicture}
\caption{Illustration of Lemma~\ref{lem:blocking_chord_persists}: A hexagonal face with triangulation $T$ containing blocking chord $(v_2, v_5)$ (red dashed). The root is $v_1$ (blue circle). Chords from the root are shown in blue, and other chords in green. The blocking chord forms triangles with vertices $v_p = v_1$ (root) and $v_q = v_3$, preventing it from being the generating chord by Case 3 of the proof.}
