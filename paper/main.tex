...
\begin{proof}
Let $(v_a, v_b)$ be a blocking chord in triangulation $T$ (i.e., $(v_a, v_b) \in S$ from a previously processed face). Since $T$ is a complete triangulation of the current face, there exist two vertices $v_p$ and $v_q$ forming triangles on opposite sides of $(v_a, v_b)$: triangles $(v_a, v_b, v_p)$ and $(v_a, v_b, v_q)$.

Recall from Definition~\ref{def:generating_chord} that the generating chord of a triangulation must:
\begin{enumerate}
    \item Have neither endpoint be the root vertex $r$
    \item Be the leftmost chord satisfying condition 1
\end{enumerate}

We analyze all possible positions of $v_p$ relative to the root vertex $r$:

\textbf{Case 1: $v_p$ is to the left of root $r$ in the vertex ordering.}

For $(v_a, v_b)$ to be flippable, both $v_a$ and $v_b$ must not be the root. However, the chord $(v_b, v_p)$ also exists in $T$ (forming the triangle $(v_a, v_b, v_p)$), and by the vertex ordering, $(v_b, v_p)$ is positioned to the left of $(v_a, v_b)$.

Since $(v_b, v_p)$ is to the left of $(v_a, v_b)$ and neither $v_b$ nor $v_p$ is the root, the chord $(v_b, v_p)$ would be the leftmost generating chord, not $(v_a, v_b)$.

Therefore, $(v_a, v_b)$ cannot be the generating chord in this case (violates the ``leftmost'' condition).

\textbf{Case 2: $v_p$ is to the right of root $r$ in the vertex ordering.}

Similarly, the chord $(v_a, v_p)$ exists in $T$ (forming the triangle $(v_a, v_b, v_p)$), and $(v_a, v_p)$ is positioned to the left of the chord that would result from flipping $(v_a, v_b)$ (namely, $(v_p, v_q)$).

Since $(v_a, v_p)$ is to the left of $(v_p, v_q)$, if $(v_a, v_b)$ were flipped, the resulting $(v_p, v_q)$ would not be the leftmost chord. This contradicts the algorithm's rule of always flipping the leftmost chord.

Therefore, $(v_a, v_b)$ cannot be the generating chord in this case (flipping it would not produce a valid child).

\textbf{Case 3: $v_p$ equals the root $r$.}
...
\begin{example}[Blocking Chord Persistence]
Consider a hexagonal face $F$ with vertices $v_1, v_2, v_3, v_4, v_5, v_6$ where chord $(v_2, v_5) \in S$ from a previous face. Suppose we choose $v_1$ as the safe root.

\textbf{Triangulation $T$ with blocking chord:}
\begin{itemize}
    \item Root chords from $v_1$: $(v_1, v_3), (v_1, v_4), (v_1, v_5)$
    \item Suppose through flipping we reach triangulation $T$ containing: $(v_1, v_3), (v_2, v_5), (v_3, v_5)$
    \item The chord $(v_2, v_5)$ is a \textbf{blocking chord} (already in $S$)
\end{itemize}

\textbf{Why $(v_2, v_5)$ cannot be the generating chord:}

The blocking chord $(v_2, v_5)$ forms two triangles: $(v_1, v_2, v_5)$ and $(v_3, v_2, v_5)$ (assuming $v_p = v_1$ and $v_q = v_3$).

\textbf{Case 1 applies:} Vertex $v_p = v_1$ is the root, so $(v_2, v_5)$ violates condition 1 of Definition~\ref{def:generating_chord} (neither endpoint can be the root). 

Wait—neither $v_2$ nor $v_5$ is the root, but $v_p = v_1$ \emph{is} the root. Let's apply Case 3:

If we tried to flip $(v_2, v_5)$ to $(v_1, v_3)$, we would create a chord from the root $v_1$. But $(v_1, v_3)$ should have been established in the root triangulation, not created through flipping.

This violates the tree structure, so $(v_2, v_5)$ cannot be the generating chord.

\textbf{Therefore:} The blocking chord $(v_2, v_5)$ persists in all descendants of $T$, making them all invalid. Pruning at $T$ is sound.
\end{example}
