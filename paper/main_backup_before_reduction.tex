...
Key notation used in this paper:
...
\hline
$G = (V, E)$ & Biconnected plane graph; $n = |V|$, $m = |E|$, $k$ faces \\
$F_i$, $n_i$ & Face $i$ with $n_i$ vertices (in counterclockwise order) \\
$T$, $T_i$, $T_{ir}$ & Triangulation (of graph / face $i$ / root of face $i$) \\
$S$ & Global chord set (tracks all added chords) \\
$(v_a, v_b)$ & Chord connecting vertices $v_a$ and $v_b$ \\
$r$ & Safe root vertex for a face \\
$C_n$ & $n$-th Catalan number: $C_n = \frac{1}{n+1}\binom{2n}{n}$ \\
\hline
\end{tabular}
\label{tab:notation}
\end{table}

Conventions: vertices in counterclockwise order; ``chord'' = internal edge added during triangulation; $O(1)$ amortized means total time $O(T)$ for $T$ triangulations.
